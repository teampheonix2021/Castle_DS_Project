...
GAME_STATUS Battle::InteractiveMode()
{
	...
	while (gameStatus == IN_PROGRESS)
	{
		...
		ActiveNumbers = Q_Active.GetLength() - FrozenNumbers;
		pGUI->UpdateInterface(CurrentTimeStep, BCastle.GetHealth(), BCastle.IsFrosted(), Q_Killed.GetLength(),
...
void Battle::ExportData(GAME_STATUS gameStatus)
{
	...
	for (int i = 0; i < TotalKills; i++)
	{
		...
		//outputting FD
		sumFirstShotDelay += EnemyPointer->GetfrstShotTime() -EnemyPointer->GetArrvTime();
		if (EnemyPointer->GetfrstShotTime() - EnemyPointer->GetArrvTime() < 10)
		{
			fout << EnemyPointer->GetfrstShotTime() - EnemyPointer->GetArrvTime() << "    ";
		}
		...
		//outputting KD
		sumKillDelay += EnemyPointer->GetKilledTime() - EnemyPointer->GetfrstShotTime();
		if (EnemyPointer->GetKilledTime() - EnemyPointer->GetfrstShotTime() < 10)
		{
			fout << EnemyPointer->GetKilledTime() - EnemyPointer->GetfrstShotTime() << "    ";
		}
		else if (EnemyPointer->GetKilledTime() - EnemyPointer->GetfrstShotTime() < 100)
		{
			fout << EnemyPointer->GetKilledTime() - EnemyPointer->GetfrstShotTime() << "   ";
		}
		else if (EnemyPointer->GetKilledTime() - EnemyPointer->GetfrstShotTime() < 1000)
		{
			fout << EnemyPointer->GetKilledTime() - EnemyPointer->GetfrstShotTime() << "  ";
		}
	...
	fout << BCastle.GetTotalDamagetaken() << endl;

	if (gameStatus == WIN)
	{
		fout << "Total Enemies = " << TotalKills << endl;
		fout << "Average First-Shot Delay = " << (double)sumFirstShotDelay / TotalKills << endl;
		fout << "Average Kill Delay = " << (double)sumKillDelay / TotalKills << endl;
	}
	else
	{
		fout << "Killed Enemies = " << TotalKills << endl;
		fout << "Alive Enemies = " << EnemyCount - TotalKills << endl;
		fout << "Average First-Shot Delay for killed = " << (double)sumFirstShotDelay / TotalKills << endl;
		fout << "Average Kill Delay for killed = " << (double)sumKillDelay / TotalKills << endl;
	}
}
